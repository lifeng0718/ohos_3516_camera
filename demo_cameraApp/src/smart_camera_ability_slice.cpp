...
#include "smart_camera_ability_slice.h"
#include <algorithm>
#include <meta_data.h>
...
namespace OHOS {
REGISTER_AS(SmartCameraAbilitySlice)
...
SmartCameraAbilitySlice::~SmartCameraAbilitySlice()
{
    gTaskView_->SetPause();
    delete gTaskView_;
...
void SmartCameraAbilitySlice::takePhoto()
{
    if (g_curButtonIdx != 1) {
        g_curButtonIdx = 1;
        bttnLeft->SetPosition(LEFT_BUTTON_X, LEFT_BUTTON_Y, LEFT_BUTTON_W, LEFT_BUTTON_H);
    ...
    cam_manager->SampleCameraCaptrue(0);
    ((SliderAnimator *)animator_)->SetStart(1);
    ((SliderAnimator *)animator_)->Start();
}

void SmartCameraAbilitySlice::takeRecord()
{
    if (g_curButtonIdx != 2) {  /* 2 record */
        g_curButtonIdx = 2;    /* 2 record */
        bttnRight->SetPosition(MID_BUTTON_X, MID_BUTTON_Y, MID_BUTTON_W, MID_BUTTON_H);
    ...
    if (g_isRecording) {
        g_isRecording = 0;    /* 0 stop */
        ((SliderAnimator *)animator_)->Stop();

        cam_manager->SampleCameraStopRecord();
        ...
        g_isRecording = 1;    /* 1 start */
        cam_manager->SampleCameraCaptrue(1);    /* 1 start */
        cam_manager->SampleCameraStartRecord(surfaceView->GetSurface());
...
void SmartCameraAbilitySlice::CameraAsrResult(int cmd)
{
...
static int CameraGetAsrResult(void *arg, const char *msg)
{
    const char *AsrMsg[] = {
        "Hi Xiaowen",
        "Nihao Wenwen"
    ...
    const int nbr = 2;
    SmartCameraAbilitySlice *camera = (SmartCameraAbilitySlice *)arg;
    if (camera == nullptr || msg == nullptr) {
        return -1;
    }

    for (int i =0; i < nbr; i++) {
        if (strcmp(msg, AsrMsg[i]) == 0) {
            camera->CameraAsrResult(i);
            break;
        }
    }
...
void SmartCameraAbilitySlice::SetHead(void)
{
...
void SmartCameraAbilitySlice::SetBottom(void)
{
...
void SmartCameraAbilitySlice::OnStart(const Want &want)
{
    AbilitySlice::OnStart(want);
    printf("SmartCameraAbilitySlice onstart ##################### \n");
    ...
    ServerSocketStart(CameraGetAsrResult, (void *)this);
    printf("after ServerSocketStart ############### \n\n");
}

void SmartCameraAbilitySlice::OnInactive()
{
    printf("SmartCameraAbilitySlice::OnInactive\n");
    AbilitySlice::OnInactive();
}

void SmartCameraAbilitySlice::OnActive(const Want &want)
{
    printf("SmartCameraAbilitySlice::OnActive\n");
...
void SmartCameraAbilitySlice::OnBackground()
{
    printf("SmartCameraAbilitySlice::OnBackground\n");
...
void SmartCameraAbilitySlice::OnStop()
{
    printf("SmartCameraAbilitySlice::OnStop\n");
    AbilitySlice::OnStop();
}
}
